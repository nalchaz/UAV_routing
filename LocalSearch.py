...
def LocalSearch2opt(sol, prob):
    ...
    while improve and count < maxCount:
        ...
        for i in range(len(sol)-3) : #for each i in the hamiltonian path
            ...
            if count > maxCount :
                return sol
            if Utils.Distance(sol[i], sol[i+1], prob) + Utils.Distance(sol[j], sol[j+1], prob) > Utils.Distance(sol[i], sol[j], prob) + Utils.Distance(sol[i+1], sol[j+1], prob) :                   
                tmp = sol[i+1]
                sol[i+1] = sol[j]
                sol[j] = tmp
                improve = True
    ...
    while improve and count < maxCount:
        ...
        for i in range(len(sol)-3) : #for each i in the hamiltonian path
            ...
            if Utils.Distance(sol[i], sol[i+1], prob) + Utils.Distance(sol[j], sol[j+1], prob) > Utils.Distance(sol[i], sol[j], prob) + Utils.Distance(sol[i+1], sol[j+1], prob) :                   
                ...
                newImprove = Utils.Distance(sol[i], sol[i+1], prob) + Utils.Distance(sol[j], sol[j+1], prob) - Utils.Distance(sol[i], sol[j], prob) + Utils.Distance(sol[i+1], sol[j+1], prob)
